...
<h2 class="title_admin">Edit Game</h2>
	<?php 
		$this->flash->show(); 
		if (validation_errors()){ echo '<div class="alert-error">'.validation_errors().'</div>';}
		$attributes = array('id' => 'add_game');
		echo form_open_multipart('admin/game_schedule/edit?id='.$this->input->get('id'), $attributes);
	?>
		<div class="add_form">
			<div class="form-group">
				<div class="leftpart"><label>Practice Game</label></div>
				<?php echo form_checkbox('practice', 1, $game_data[0]['practice']); ?>
			</div>
			<div class="form-group">
				<div class="leftpart"><label>Date (mm/dd/yyyy) *</label></div>
				<div class="rightpart"><?php $data = array('name' => 'date', 'id' => 'date', 'onchange' => 'onchangeTest()', 'value' => set_value('date', $game_data[0]['date'])); echo form_input($data); ?></div>

			</div>
			<div class="form-group">
				<div class="leftpart"><label>Time (hh:mm) *</label></div>
				<div class="rightpart"><?php $data = array('name' => 'time', 'id' => 'time', 'value' => set_value('time', $game_data[0]['time'])); echo form_input($data); ?>24 hour time</div>
			</div>
			<div class="form-group">
				...
				<div class="rightpart">
					<?php
						...
						endforeach;
						echo form_dropdown('division_id', $options, set_value('division_id', $game_data[0]['division_id']), $class);
					?>
				</div>
			</div>
			<div class="form-group">
				...
				<div class="rightpart">
					<?php
						$class = 'class="select_wide visitor_teams_dd" id="home_team"';
						$options_teams[''] =  'Select';
						foreach($teams as $item) :
						$options_teams[$item['id']] =  htmlspecialchars($item['name']);
						endforeach;
						echo form_dropdown('home_team_id', $options_teams, set_value('home_team_id', $game_data[0]['home_team_id']), $class);
					?>
				</div>
			</div>
			<div class="form-group">
				...
				<div class="rightpart">
					<?php
						$setting_team = 'class="select_wide visitor_teams_dd" id="visitor_team"';
					echo form_dropdown('visitor_team_id', $options_teams, set_value('visitor_team_id', $game_data[0]['visitor_team_id']), $setting_team);
					?>
				</div>
			</div>
			<div class="form-group">
				...
				<div class="rightpart">
					<?php
						...
						$options[$item['id']] =  htmlspecialchars($item['name']);
						endforeach;
						echo form_dropdown('location_id', $options, set_value('location_id', $game_data[0]['location_id']), $class);
					?>
				</div>
			</div>
			<div class="form-group">
